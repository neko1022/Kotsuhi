...
if is_admin:
        ...
        if not df_all.empty:
            ...
            for idx, row in user_summary.iterrows():
                ...
                if show_detail:
                    ...
                    st.markdown(f'<table class="table-style"><thead><tr><th class="col-date">日付</th><th class="col-route">区間</th><th class="col-dist">距離</th><th class="col-highway">高速代</th><th class="col-total">合計</th></tr></thead><tbody>{rows_html}</tbody></table>', unsafe_allow_html=True)
    ...
    if selected_user != "選択してください":
        ...
        if user_pwd == USER_PASS:
            ...
            st.markdown(f'<div class="header-box"><p class="total-label">{selected_user} さんの今月合計</p><p class="total-a">{int(filtered_df["合計金額"].sum()):,} 円</p></div>', unsafe_allow_html=True)
            ...
            with c2:
                # 3. 走行距離
                dist_str = st.text_input("走行距離 (km)", placeholder="数字のみ入力")
                # 4. 高速道路料金
                highway_str = st.text_input("高速道路料金 (円)", placeholder="数字のみ入力", value="0")
            ...
            dist_val = get_clean_num(dist_str)
            highway_val = get_clean_num(highway_str)
            ...
            if st.button("この内容で登録する", use_container_width=True):
                if dist_val > 0 or highway_val > 0:
                    ...
                    st.warning("距離または高速代を入力してください。")

            if not filtered_df.empty:
                st.markdown("---")
                st.markdown('<div class="history-header">🗓️ 走行明細履歴</div>', unsafe_allow_html=True)
